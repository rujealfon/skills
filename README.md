...
Each skill records two version signals in its own `README.md` and `CHANGELOG.md`, never in `SKILL.md`'s frontmatter. `SKILL.md` loads into context every time the skill triggers, and neither number affects triggering:
...
- **Tracks.** The major (or, for pre-1.0 packages, minor) line of the upstream package the skill was last verified against, plus the date of that check. It tells you whether a skill is due for a refresh: if the installed package has moved to a new major/minor since the "tracks" date, re-verify the skill's guidance against the current docs and bump both the tracked line and the skill version when you update it.
...
The script reads the `- Tracks:` lines out of every skill README, which stay the single source of truth, and compares them against what npm currently publishes. Each package lands in one of four states:
