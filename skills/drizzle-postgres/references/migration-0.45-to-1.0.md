...
This is the dangerous change. A project that upgrades to 1.0 while leaving `casing: 'snake_case'` in its config loses the option with no error. Every `pgTable` with unnamed columns starts emitting camelCase identifiers, which don't match the existing database. This surfaces as `column "firstName" does not exist` at runtime, not as a type error. When migrating, convert the tables *before* running anything against a real database, and diff a generated migration to confirm no column renames appear.

`drizzle.config.ts` still has an `introspect.casing` option (`'camel' | 'preserve'`). That option is unrelated. It controls how `drizzle-kit pull` names properties in the TypeScript it generates, not how your schema maps to SQL.
...
Build a fresh object per table, or a factory function when the same columns repeat.
...
The differences that matter most: `defineRelations` takes the whole schema and returns one object keyed by table (rather than one exported `relations()` call per table), and the field mapping is `from`/`to` rather than `fields`/`references`.
...
Compared with 0.45.x: `casing` is gone (see above), the RQBv1 `schema` option is gone (`db.query` is wired through `relations` only), and `jit` is new. It toggles the just-in-time compiled row mappers (off by default; uses a `Function` constructor, so it turns off under CSP `jitless`).
...
These don't break existing 0.45.x code, but they change the best choice on a 1.0 project. Check the installed types before relying on specifics.

- **Codecs.** Driver-aware cast/normalize layer (regular SELECT, JSON/RQB context, INSERT params). Each driver enables them by default; override via `drizzle(client, { codecs: { ... } })`. Deep reference: [Codecs](https://orm.drizzle.team/docs/codecs).
...
1. Confirm the installed version is 1.0 (`node -p "require('drizzle-orm/package.json').version"`). Don't migrate a project still on 0.45.x.
